...
resource "aws_launch_template" "node" {
  ...
  user_data = base64encode(templatefile("${path.module}/templates/userdata.sh.tpl", {
    ...
    node_labels         = length(lookup(each.value, "labels", {})) > 0 ? join(",", [for k, v in each.value.labels : "${k}=${v}"]) : ""
    node_taints         = length(lookup(each.value, "taints", [])) > 0 ? join(",", [for t in each.value.taints : "${t.key}=${t.value}:${t.effect}"]) : ""
  }))
